...
async function getListOfResults(firstTime, fromFilters) {
    ...
    const url = '/General/AdminTimeOffApprovals/GetRequests?model=' + encodeURIComponent(queryString);
...
async function confirmReject() {
    const comment = document.getElementById('admin-rejection-comment-textarea').value.trim();
    if (!comment) {
...
async function processDecision(status, rejectionComment) {
    ...
    try {
        const response = await fetch('/General/AdminTimeOffApprovals/ApproveReject', {
            method: 'POST',
            headers: {
        ...
        if (response.ok && result.success) {
            ...
            hideModal('modal-admin-approve-request');
            hideModal('modal-admin-reject-request');
            getListOfResults(true, false);
