...
function renderResults(data) {
    ...
    exportExcelBtn.textContent = 'Export to CSV';
    ...
    exportPdfBtnContainer.appendChild(exportExcelBtn);
...
function exportToExcel(resultsToPrint) {
    ...
    // Map data with the correct column names and appropriate formats
    const formattedData = resultsToPrint.map(({
        actionDate, timeFrom, timeTo, quantity,
        consultantName, movementType, notes, projectName
    }) => ({
        "Date": formatDate(actionDate),
        "Hours In": formatTime(timeFrom),
        "Hours Out": formatTime(timeTo),
        "Total Hours": quantity,
        "Consultant": consultantName,
        "Hours Type": movementType,
        "Notes": notes,
        "Project Name": projectName
    }));

    // Create a worksheet and an Excel workbook
    ...
    const columnWidths = Object.keys(formattedData[0]).map(key => ({
        wch: Math.max(...formattedData.map(row => row[key]?.toString().length || 10), key.length)
    }));
    worksheet["!cols"] = columnWidths;

    // Generate the Excel file and trigger the download
    XLSX.writeFile(workbook, "Movements_Report.xlsx");
}
