    ...
    public class PaymentSheetsController : Controller
    {
        ...
        public async Task<IActionResult> GetReportToMakePayment(int consultantId, DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                var activeProjects = await _unitOfWork.ProjectConsultantAssigned.GetProjectsInfoWhereConsultantIsActiveInPeriod(consultantId,
                    startDate, endDate);
                ...
                {
                    return NotFound(new { error = "Default project not found." });
                }
                ...
                {
                    var projectMovements = await _unitOfWork.ReportingMyTimeMovement.GetApprovedMovementsWhereConsultant(consultantId, project.ProjectId,
                        startDate, endDate);
                ...
                {
                    var holidays = await _unitOfWork.ConsultantHoliday.GetHolidaysDatesWhereConsultantInPeriodAsync(startDate, endDate,
                   consultant.ConsultantHolidayId);
                ...
                //Add Benefits
                var benefits = await _unitOfWork.ConsultantReimbursedBenefit.GetApprovedBenefitsWhereConsultantInThePeriod(consultantId,
                    startDate, endDate);
                ...
                //Add Interviews
                var interviews = await _unitOfWork.Interview.GetApprovedInterviewsWhereConsultantInThePeriod(consultantId,
                    startDate, endDate);
                ...
                var debitsAndCredits = await _unitOfWork.ConsultantPaymentsDebitsCredits.GetApprovedDebitsCreditsWhereConsultantInThePeriod(consultantId,
                    startDate, endDate);
