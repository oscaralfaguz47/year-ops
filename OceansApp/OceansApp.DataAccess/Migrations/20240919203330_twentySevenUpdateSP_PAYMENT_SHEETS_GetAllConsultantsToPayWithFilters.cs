...
{
    /// <inheritdoc />
    public partial class twentySevenUpdateSP_PAYMENT_SHEETS_GetAllConsultantsToPayWithFilters : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            @AccountsPayableStatusName VARCHAR(20),
            @ProjectId INT,
            @PaymentPeriod INT,
            ...
            WITH AccountsPayableStatus AS (
                SELECT
                    ...
                    CASE
                        -- If there are no records in ACCOUNTS_PAYABLE, return 'Pending'
                        WHEN COUNT(AP.ConsultantId) = 0 THEN 'Pending'
                        -- If there are multiple distinct statuses, return 'Sent to be paid'
                        WHEN COUNT(DISTINCT TS.Name) > 1 THEN 'Sent to be paid'
                        -- If all statuses are 'Paid', return 'Paid'
                        WHEN MAX(TS.Name) = 'Paid' AND MIN(TS.Name) = 'Paid' THEN 'Paid'
                        -- Otherwise, return the single status found
                        ELSE MAX(TS.Name)
                ...
                WHERE (AP.StartDatePeriod >= @StartDate OR AP.StartDatePeriod IS NULL) -- Handle cases with no AP records
            ...
            ActiveConsultants AS (
                ...
                AND (PCAH.MonthlySalary > 0 OR PCAH.HourlySalary > 0) -- Ensure salary is not zero
            ),
            ...
            PagedResults AS (
                ...
                WHERE 
            		...
            		AND (@AccountsPayableStatusName IS NULL OR APS.PaymentStatus = @AccountsPayableStatusName)  -- Add the filter by PaymentStatus
                    AND NOT EXISTS (
