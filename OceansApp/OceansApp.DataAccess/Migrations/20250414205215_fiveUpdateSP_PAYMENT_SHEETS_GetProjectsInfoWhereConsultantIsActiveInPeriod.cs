...
{
    /// <inheritdoc />
    public partial class fiveUpdateSP_PAYMENT_SHEETS_GetProjectsInfoWhereConsultantIsActiveInPeriod : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            WITH ActiveProjects AS (
                ...
                ON LastHistory.ProjectConsultantAssignedId = PCAH.ProjectConsultantAssignedId 
                AND LastHistory.MaxActionDate = PCAH.ActionDate
                WHERE PCAH.IsActive = 1  -- Ensure consultant is active
                AND (PCAH.MonthlySalary > 0 OR PCAH.HourlySalary > 0) -- Ensure non-zero salaries
            ),
            ...
            RecentProjectHistory AS (
                SELECT
                    ...
                    PCAH.IsActive,  -- Include IsActive status here for final filtering
            ...
            SELECT
                ...
                RPH.PartnerId,
                RPH.AccessToTrackingTool,
                PA.Name AS PartnerName,
                RPH.PartnerPaysBenefits,
                RPH.NumHoursForHoliday
            FROM (
            ...
            LEFT JOIN PARTNERS PA ON AllProjects.ProjectId = PA.PartnerId
            ...
            AND RPH.IsActive = 1  -- Ensure that only active projects are included
            AND (RPH.MonthlySalary > 0 OR RPH.HourlySalary > 0) -- Ensure projects with non-zero salaries
            AND NOT EXISTS (
                -- Exclude projects with disabled tracking records
                SELECT 1
                FROM PROJECTS_CONSULTANTS_PERIODS_DISABLED_TRACKINGS PCPDT
                WHERE P.ProjectId = PCPDT.ProjectId
                AND AllProjects.ConsultantId = PCPDT.ConsultantId
                AND PCPDT.StartPeriodDate <= @EndDate
                AND PCPDT.EndPeriodDate >= @StartDate
            )
            ORDER BY P.Name;
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            var spOriginal = @"CREATE PROCEDURE SP_PAYMENT_SHEETS_GetProjectsInfoWhereConsultantIsActiveInPeriod
            @StartDate DATE,
            @EndDate DATE,
            @ConsultantId INT
            AS
            ...
            migrationBuilder.Sql("DROP PROCEDURE IF SP_PAYMENT_SHEETS_GetProjectsInfoWhereConsultantIsActiveInPeriod");
