...
{
    /// <inheritdoc />
    public partial class thirtyfiveUpdateSP_PAYMENT_SHEETS_GetAllConsultantsToPayWithFiltersDebitCreditReimbursement : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // NHP-2.1: surface project-less consultants who have a non-rejected debit/credit OR
            // reimbursement (and zero active assignments), in addition to the M1 interview case.
            // The single project-less row, pricing (stored amounts), and pay flow are reused from
            // M1; only the ProjectLessConsultants surfacing predicate changes here.
        ...
        {
            // Restore the M1 procedure: project-less rows surfaced only for a non-rejected interview.
            var sp = @"CREATE PROCEDURE SP_PAYMENT_SHEETS_GetAllConsultantsToPayWithFilters
            @SearchText NVARCHAR(100),
            @StartDate DATE,
            @EndDate DATE,
