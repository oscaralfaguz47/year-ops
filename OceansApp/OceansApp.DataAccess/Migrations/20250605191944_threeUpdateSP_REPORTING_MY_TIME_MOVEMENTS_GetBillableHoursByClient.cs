...
{
    /// <inheritdoc />
    public partial class threeUpdateSP_REPORTING_MY_TIME_MOVEMENTS_GetBillableHoursByClient : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var sp = @"CREATE PROCEDURE SP_REPORTING_MY_TIME_MOVEMENTS_GetBillableHoursByClient
             ...
             SELECT 
                 ...
                 MB.MovementTypeId,
                 MAX(CASE 
                     WHEN LH.MonthlyClientRate > 0 AND LH.HourlyClientRate = 0 AND MB.TotalHours >= LH.MonthlyClientRateNumDays THEN 1
                     WHEN LH.MonthlyClientRate > 0 AND LH.HourlyClientRate = 0 AND MB.TotalHours < LH.MonthlyClientRateNumDays THEN MB.TotalHours
                     WHEN MB.MovementTypeName = 'Normal Hours'
                          AND MB.TotalHours > MB.LimitNumHoursForOverTime
                          AND MB.LimitNumHoursForOverTime > 0 THEN MB.LimitNumHoursForOverTime
                     ELSE MB.TotalHours
                 END) AS TotalHours,
                 MAX(CASE 
                     WHEN LH.MonthlyClientRate > 0 AND LH.HourlyClientRate = 0 AND MB.TotalHours >= LH.MonthlyClientRateNumDays THEN LH.MonthlyClientRate
                     WHEN LH.MonthlyClientRate > 0 AND LH.HourlyClientRate = 0 AND MB.TotalHours < LH.MonthlyClientRateNumDays THEN LH.MonthlyClientRate / NULLIF(LH.MonthlyClientRateNumDays, 0)
                 ...
                 MAX(CASE 
                     WHEN LH.MonthlyClientRate > 0 AND LH.HourlyClientRate = 0 THEN MB.TotalHours
                     ELSE NULL
                 END) AS ReportedHoursIfMonthlyClientRate,
                 MAX(CASE 
                     WHEN LH.MonthlyClientRate > 0 AND LH.HourlyClientRate = 0 THEN LH.MonthlyClientRate / NULLIF(LH.MonthlyClientRateNumDays, 0)
                     ELSE NULL
                 END) AS UnitHourPriceIfMonthlyClientRate,
                 MIN(CASE 
