...
{
    public partial class fourthUpdateSP_CONSULTANT_DETAILS_GetAllConsultantsWithFilters : Migration
    {
   ...
   AND
    ...
    OUTER APPLY (
        SELECT STRING_AGG(P.Name, '; ') AS ConsultantProjects
        FROM PROJECTS_CONSULTANTS_ASSIGNED PA
        JOIN PROJECTS P ON PA.ProjectId = P.ProjectId
        WHERE PA.ConsultantId = CD.ConsultantId
        AND PA.IsActive = 1
    ) AS Projects
    WHERE 
    (
