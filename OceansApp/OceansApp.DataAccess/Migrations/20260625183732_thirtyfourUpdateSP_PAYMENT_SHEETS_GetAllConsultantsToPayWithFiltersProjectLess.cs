...
{
    /// <inheritdoc />
    public partial class thirtyfourUpdateSP_PAYMENT_SHEETS_GetAllConsultantsToPayWithFiltersProjectLess : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Surface project-less consultants (a non-rejected interview in the period and ZERO active
            // assignments) as a single project-less row (ProjectId NULL, ProjectName '—', hours 0).
            // They are unioned in only when no project filter is applied; pay-period and name filters
            // still apply. Consultants with an active assignment are unaffected (no duplicate row).
            ...
            -- Consultants with a non-rejected interview in the period and ZERO active assignments.
            -- These appear on the sheet as a single project-less row.
        ...
        {
            // Restore the previous version of the stored procedure (without project-less rows).
            var sp = @"CREATE PROCEDURE SP_PAYMENT_SHEETS_GetAllConsultantsToPayWithFilters
            @SearchText NVARCHAR(100),
            @StartDate DATE,
            @EndDate DATE,
            ...
            WITH AccountsPayableStatus AS (
                ...
                LEFT JOIN TRANSACTION_STATUSES TS ON AP.TransactionStatusId = TS.TransactionStatusId
                WHERE AP.Voided = 0
                  AND (AP.StartDatePeriod >= @StartDate OR AP.StartDatePeriod IS NULL)
                  AND (AP.EndDatePeriod <= @EndDate OR AP.EndDatePeriod IS NULL)
                GROUP BY C.ConsultantId
            ),
            SubmissionsCounts AS (
                SELECT
