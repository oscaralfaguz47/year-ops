...
{
    /// <inheritdoc />
    public partial class sixUpddateSP_PROJECTS_CONSULTANT_ASSIGNED_GetConsultantStatusInTheProject : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            -- CTE to determine the most recent ParticipatesInOnCalls and Tracking Tool Names
            ParticipatesInOnCallsStatus AS (
                SELECT
                    ...
                    (
                        SELECT TOP 1 PCAH.PrimaryReportTrackingToolName
                        FROM PROJECTS_CONSULTANTS_ASSIGNED_HISTORY PCAH
                        WHERE PCAH.ProjectConsultantAssignedId = PCA.ProjectConsultantAssignedId
                        AND PCAH.ActionDate <= @EndDate
                        ORDER BY PCAH.ActionDate DESC, PCAH.Id DESC
                    ) AS PrimaryReportTrackingToolName,
                    (
                        SELECT TOP 1 PCAH.SecondReportTrackingToolName
                        FROM PROJECTS_CONSULTANTS_ASSIGNED_HISTORY PCAH
                        WHERE PCAH.ProjectConsultantAssignedId = PCA.ProjectConsultantAssignedId
                        AND PCAH.ActionDate <= @EndDate
                        ORDER BY PCAH.ActionDate DESC, PCAH.Id DESC
                    ) AS SecondReportTrackingToolName
                FROM PROJECTS_CONSULTANTS_ASSIGNED PCA
            ),
            ...
            SELECT 
                ...
                PIOC.PrimaryReportTrackingToolName,
                PIOC.SecondReportTrackingToolName,
                ISNULL(PAS.ProjectIsActiveInThePeriod, 1) AS ProjectIsActiveInThePeriod,
                ISNULL(P.IsBillable, 0) AS ProjectIsBillable,
            ...
            LEFT JOIN PROJECTS P ON PCA.ProjectId = P.ProjectId
