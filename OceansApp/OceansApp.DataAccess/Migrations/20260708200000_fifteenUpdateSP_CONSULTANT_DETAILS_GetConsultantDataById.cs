...
namespace OceansApp.DataAccess.Migrations
{
    ...
    public partial class fifteenUpdateSP_CONSULTANT_DETAILS_GetConsultantDataById : Migration
    {
        ...
        {
            // LEFT JOIN the role tables so consultants with no role can still be loaded,
            // and exclude the Weekly Pulse Participant role so UserRole is always the
            // single job role (a participant would otherwise produce a second row).
        ...
        LEFT JOIN UserRoles UR ON CD.UserId = UR.UserId
            AND UR.RoleId NOT IN (SELECT Id FROM Roles WHERE Name = 'Weekly Pulse Participant')
