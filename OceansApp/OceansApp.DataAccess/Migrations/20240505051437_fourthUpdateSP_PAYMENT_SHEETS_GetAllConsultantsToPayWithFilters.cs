...
{
    /// <inheritdoc />
    public partial class fourthUpdateSP_PAYMENT_SHEETS_GetAllConsultantsToPayWithFilters : Migration
    {
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            FROM (
                ...
                AND (@SearchText IS NULL 
                            ...
                            OR LOWER(CONCAT(U.Name, ' ', U.LastName)) LIKE '%' + LOWER(@SearchText) + '%')
                AND (@PaymentPeriod IS NULL OR CD.PaymentPeriod = @PaymentPeriod)
                AND (SELECT COUNT(*) FROM PROJECTS_CONSULTANTS_ASSIGNED_HISTORY PCAH
                INNER JOIN PROJECTS_CONSULTANTS_ASSIGNED_HISTORY_ACTIONS HA ON PCAH.ActionId = HA.ActionId
			    WHERE CONVERT(DATE, PCAH.ActionDate) <= @StartDateFormatted ) > 0
                AND (
                    EXISTS (
                        SELECT 1
                        FROM (
