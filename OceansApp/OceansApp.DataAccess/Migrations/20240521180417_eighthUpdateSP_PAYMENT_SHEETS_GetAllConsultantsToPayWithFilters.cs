...
{
    /// <inheritdoc />
    public partial class eighthUpdateSP_PAYMENT_SHEETS_GetAllConsultantsToPayWithFilters : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
                        ...
                        FROM (
                            SELECT TOP(1)
                                CASE
                                    ...
                                    WHEN HA.Name = 'Consultant Deactivated' AND CONVERT(DATE, PCAH.ActionDate) = @StartDateFormatted THEN 'Active'
            ...
            ActiveProjects AS (
                ...
                JOIN PROJECTS P ON PCA.ProjectId = P.ProjectId
                WHERE
                    CONVERT(DATE, PCAH.ActionDate) <= @EndDateFormatted AND (
                        EXISTS (
                            SELECT 1
                            FROM (
                                SELECT TOP(1)
            ...
            SELECT
                ...
                COALESCE(AP.NumProjectsIsActive, 0) AS NumProjectsIsActive,
                PCA.AccessToTrackingTool
            FROM PROJECTS_CONSULTANTS_ASSIGNED_HISTORY PCAH
            JOIN PROJECTS_CONSULTANTS_ASSIGNED PCA ON PCAH.ProjectConsultantAssignedId = PCA.ProjectConsultantAssignedId
            JOIN PROJECTS_CONSULTANTS_ASSIGNED_HISTORY_ACTIONS HA ON PCAH.ActionId = HA.ActionId
            ...
            JOIN Users U ON CD.UserId = U.Id
            LEFT JOIN SubmissionsCounts SC ON CD.ConsultantId = SC.ConsultantId AND P.ProjectId = SC.ProjectId
            LEFT JOIN ActiveProjects AP ON CD.ConsultantId = AP.ConsultantId
            ...
            WHERE 
                ...
                (@PaymentPeriod IS NULL OR CD.PaymentPeriod = @PaymentPeriod) AND
                CONVERT(DATE, PCAH.ActionDate) <= @EndDateFormatted AND (
                    EXISTS (
                        SELECT 1
                        FROM (
            ...
            GROUP BY CONCAT(U.Name, ' ', U.LastName), CD.ConsultantId, P.ProjectId, P.Name, RS.SubmissionId, RS.TransactionStatusId, TS.Name, RS.SubmissionDate, RS.LastSubmissionDate, NAS.NumApprovedSubmissions, AP.NumProjectsIsActive, PCA.AccessToTrackingTool
            ORDER BY 
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            FROM (
                ...
                FROM 
                    ...
                    INNER JOIN TRANSACTION_STATUSES SCTS ON SCRMTS.TransactionStatusId = SCTS.TransactionStatusId
                WHERE SCTS.Name = 'Approved'
                GROUP BY ConsultantId, StartPeriodDate, EndPeriodDate
            ),
            ActiveProjects AS (
                SELECT
                    PCA.ConsultantId,
            ...
            SELECT
                ...
                RS.SubmissionDate,
                RS.LastSubmissionDate,
                COALESCE(SC.NumSubmissions, 0) AS NumApprovedSubmissions,
                COALESCE(AP.NumProjectsIsActive, 0) AS NumProjectsIsActive
            FROM PROJECTS_CONSULTANTS_ASSIGNED_HISTORY PCAH
            JOIN PROJECTS_CONSULTANTS_ASSIGNED PCA ON PCAH.ProjectConsultantAssignedId = PCA.ProjectConsultantAssignedId
            JOIN PROJECTS_CONSULTANTS_ASSIGNED_HISTORY_ACTIONS HA ON PCAH.ActionId = HA.ActionId
            ...
            LEFT JOIN TRANSACTION_STATUSES TS ON RS.TransactionStatusId = TS.TransactionStatusId
            JOIN Users U ON CD.UserId = U.Id
            LEFT JOIN SubmissionsCounts SC ON CD.ConsultantId = SC.ConsultantId 
            AND SC.StartPeriodDate = @StartDateFormatted AND SC.EndPeriodDate = @EndDateFormatted
            LEFT JOIN ActiveProjects AP ON CD.ConsultantId = AP.ConsultantId
            WHERE 
                (@ProjectId IS NULL OR P.ProjectId = @ProjectId) AND
                (@TransactionStatusId IS NULL OR (TS.TransactionStatusId = @TransactionStatusId AND TS.TransactionStatusId IS NOT NULL)) AND
            ...
            GROUP BY CONCAT(U.Name, ' ', U.LastName), CD.ConsultantId, P.ProjectId, P.Name, RS.SubmissionId, TS.Name, RS.SubmissionDate, RS.LastSubmissionDate, SC.NumSubmissions, AP.NumProjectsIsActive
            ORDER BY 
                CASE WHEN @FieldToOrder = 'ConsultantName' AND @DirectionOrder = 'ASC' THEN CONCAT(U.Name, ' ', U.LastName) END ASC,
                CASE WHEN @FieldToOrder = 'ConsultantName' AND @DirectionOrder = 'DESC' THEN CONCAT(U.Name, ' ', U.LastName) END DESC,
                CASE WHEN @FieldToOrder = 'ProjectName' AND @DirectionOrder = 'ASC' THEN P.Name END ASC,
