...
{
    /// <inheritdoc />
    public partial class thirdUpdateSP_CONSULTANT_PAYMENTS_DEBITS_CREDITS_GetAllPaymentsDebitsCreditsWithFilters : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            LEFT JOIN CONSULTANT_DETAILS CUpdate ON CPDC.ConsultantIdLastUpdatedBy = CUpdate.ConsultantId
            LEFT JOIN Users UUp ON CUpdate.UserId = UUp.Id
            WHERE (@SearchText IS NULL 
                   OR LOWER(UC.Name) LIKE '%' + LOWER(@SearchText) + '%'
                   OR LOWER(UC.LastName) LIKE '%' + LOWER(@SearchText) + '%'
                   OR (LOWER(@SearchText) LIKE '%' + LOWER(UC.Name) + '%' AND LOWER(@SearchText) LIKE '%' + LOWER(UC.LastName) + '%')
              ...
              AND ((@StartDate IS NULL AND @EndDate IS NULL) 
                   OR (CPDC.ActionDateWithinFortnight BETWEEN @StartDate AND @EndDate));
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            var spOriginal = @"CREATE PROCEDURE SP_CONSULTANT_PAYMENTS_DEBITS_CREDITS_GetAllPaymentsDebitsCreditsWithFilters
            @SearchText NVARCHAR(255),
            @StartDate DATE,
            @EndDate DATE,
            @TransactionStatusId INT,
            ...
            migrationBuilder.Sql("DROP PROCEDURE IF SP_CONSULTANT_PAYMENTS_DEBITS_CREDITS_GetAllPaymentsDebitsCreditsWithFilters");
